...
const MainPage = ({ availableTags, notes }: MainProps) => {
  ...
  const filtredNotes = useMemo(
    () =>
      notes.filter((note) => {
        return (
          // Return notes if the note's title contains the text I searched for
          (note.title === "" ||
            note.title.toLowerCase().includes(title.toLowerCase())) &&
          // If all the tags I selected are in the note, bring the note
    ...
    <div className="container py-5">
      ...
      <Form>
        <Row>
          ...
          <Col>
            <Form.Group>
              <Form.Label>Seacrh by Tags</Form.Label>
              <ReactSelect
                onChange={(tags) => {
                  setSelectedTags(
                    tags.map((tag) => ({
